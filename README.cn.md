...
TAFFISH `0.10.0` 是当前公开 release。源码、ASDF 系统、源码树开发文档、release
载荷、贡献指南和安全策略都位于
[taffish/taffish](https://github.com/taffish/taffish)。

`0.10.0` 增加了面向已安装 Hub app 的本地维护命令：`taf install --all`、
`taf outdated`、`taf upgrade` 和 `taf prune`。这些命令默认保持保守：规划命令
会先展示预期变更，真正写入或删除本地 app 需要显式确认，例如 `--yes`。上一版
`0.9.0` 引入的 runtime 参数策略仍然是当前 `.taf` 标签 backend-specific container
arguments 和本机 runtime 环境变量的使用模型。
