...
| [容器化 app 最佳实践](zh/container-apps.cn.md) | 容器专题。覆盖 Dockerfile 设计、smoke 元数据、运行时挂载、GHCR、Docker/Podman 测试、backend 一致性，以及 TAFFISH `0.9.0` runtime 参数策略。 |
...
TAFFISH `0.9.0` 是当前公开 release。源码、ASDF 系统、源码树开发文档、release
载荷、贡献指南和安全策略都位于
[taffish/taffish](https://github.com/taffish/taffish)。

`0.9.0` 增加了 `.taf` 标签中的结构化 backend-specific container runtime
arguments、本机 backend runtime 参数环境变量、外部 binary-level 测试，并刷新带签名
checksum 的 release 载荷。
