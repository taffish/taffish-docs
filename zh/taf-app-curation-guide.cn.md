...
`docs/help.md` 是用户运行 `taf-xxx --help` 看到的内容。它应该像终端命令手册，而不是
项目介绍。用户只看 help 就应该能了解基本运行路径、常用选项、输入需求、关键输出和
简单/固定案例。更长的背景、领域意义、完整数据格式说明和维护说明应放在 README 或
中心文档中。
新建 app 或大幅重写文档时，优先从 `repos/apps/templates/app-docs/` 的共享模板起步，
再按具体 app 裁剪。

示意结构：

共享模板是正式起点。下面的大纲只是展示终端 help 的风格，不应优先于
`repos/apps/templates/app-docs/` 模板。
...
- Help 应该短、准、能直接复制命令。
- 有需要时可以放 README、GitHub 仓库或详细手册的纯文本 URL；不要使用 Markdown
  链接语法，因为 help 会直接输出到终端。
...
README 面向 GitHub 页面、人工审核和深度用户，比 `docs/help.md` 更适合说明包结构。
它应承载背景信息、领域作用、依赖和版本选择、上下游流程、详细输入格式、输出语义、
资源建议、故障定位、citation/license 和维护检查。不要把基本运行所需的信息只放在
README；help 仍应能支撑普通命令行使用。
