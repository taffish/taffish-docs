...
已经存在 upstream object 的 `upstream.license`。
...
如果 `taffish.toml` 没有提供任何有效 upstream 字段，则 version record 中不出现 `upstream`。index metadata override 只能在 upstream object 已经存在后补充 `upstream.license`。
