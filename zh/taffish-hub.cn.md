...
官方 TAFFISH Hub 的 app 发布和维护采用精修审核，而不是开放自助提交。只有获得授权的
`taffish` GitHub 组织成员才能创建或发布官方 app 仓库。开发者希望把 app 纳入官方
Hub 时，应联系维护者完成 review、ownership 分配，并根据情况加入组织或由维护者协助发布。

这样设计是为了让 app 元数据、版本 tag、容器镜像、许可证、upstream 来源和依赖关系
保持一致。随着维护者和外部贡献增加，review、ownership 和贡献策略也应进一步显式化。
...
`repos/apps/` 保存各个独立发布 taf-app 仓库的本地工作树。`hubctl/`、`update/` 和
`apps-archive/` 属于维护者运维工具与记录，不是用户直接消费的仓库。
